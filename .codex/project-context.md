...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. Work Package 1 is catalogue metadata only and does not authorize paid access or enforce allowances.
