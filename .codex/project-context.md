...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. `server/billing/subscription_service.py` is the only effective-plan lifecycle authority and falls back conservatively to Free; `server/billing/entitlement_service.py` owns feature/model/file decisions and future reservation quantities. `BILLING_ENABLED=false` is the safe default until verified Stripe webhook synchronization is deployed. Request-route enforcement and atomic metering remain later packages, and verified Stripe webhooks must ultimately be authoritative for paid state.
...
- Billing repository functions in `db/billing_repository.py` do not commit. Their callers own transaction boundaries, and counter locks must be acquired in deterministic meter-key order before later metering code checks or mutates allowance state.
