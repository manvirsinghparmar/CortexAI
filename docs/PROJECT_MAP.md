...
- Credit arithmetic and deterministic token estimates: `server/billing/credit_calculator.py`, `server/billing/credit_estimator.py`; Ask/Compare/Optimize/Cortex Analysis authorization, Smart candidate affordability, supplemental settlement, and adjustment-ledger reconciliation: `server/billing/enforcement_service.py`; committing route adapters: `server/persistence.py`; request integration: `server/routes/chat.py`, `server/routes/compare.py`, `server/routes/optimize.py`, `server/routes/cortex_analysis.py`
