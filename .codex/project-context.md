...
- On mobile, those synchronized native selects overlay the styled picker controls to provide reliable device-native touch selection; desktop retains the custom accessible listbox.
...
- React frontend assistant responses use `frontend-react/src/components/results/` with progressive Ask/Compare stream rendering, response-scoped source targets, code-block copy controls, sanitized provider errors, and explicit `Jump to latest` behavior without active auto-follow.
...
- Desktop React Compare keeps model headers/actions fixed within equal-height columns and gives each response body an independent vertical scrollbar; the comparison grid owns horizontal overflow. Mobile Compare uses a segmented model switcher and renders one natural-height response card at a time.
...
- Mobile response cards collapse per-model latency, token, and cost metadata behind a `Details` control by default; desktop keeps it visible.
