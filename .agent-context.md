...
- **Quick Test**: `python quick_test_optimizer.py` - Quick testing utility
...
CLI Layer (main.py)                        → User I/O
    ↓
FastAPI Layer (server/)                    → HTTP API (auth, validation)
    ↓                                         /health, /v1/chat, /v1/compare
    ↓                                         asyncio.to_thread() wrapper
CortexOrchestrator (orchestrator/core.py)  → Business logic (shared)
    ↓
Database Layer (db/)                      → Persistent storage (NEW)
    ↓
Research Tools (tools/web/)               → Web research capabilities (NEW)
    ↓
...
- **Quick Test**: `python quick_test_optimizer.py`
...
- Features: Token tracking, cost calculation, session stats, **structured JSON logging**, **FastAPI REST API**, **database persistence**, **web research tools**
...
.github/                 → GitHub workflows and configuration (NEW)
  └── workflows/
      └── ci.yml         → Continuous integration workflow (NEW)
        → Ruff linting, Black formatting, MyPy type checking
        → Pytest unit tests, pip-audit security scanning
        → Gitleaks secrets detection, quality gates
...
db/engine.py             → Database engine configuration (NEW)
db/repository.py          → Data access layer (NEW)
db/session.py             → Database session management (NEW)
db/tables.py             → Database table definitions (NEW)
...
tools/web/               → Research and web tools (NEW)
...
docs/                    → Documentation (NEW organization)
  ...
  ├── DATABASE_INTEGRATION_COMPLETE.md → Database docs
  ├── FASTAPI_README.md  → FastAPI documentation
  ├── LOGGING.md         → Logging documentation
  ├── TAVILY_INTEGRATION.md → Tavily integration docs
  └── ...                → Other documentation
tests/                   → Test suite
  ├── test_api.py         → API test script
  ├── test_conversation.py → Conversation tests
  ├── test_fastapi_contract_and_guardrails.py → FastAPI tests
  └── ...                → Other tests
```

## Recent Changes
- **🗄️ DATABASE INTEGRATION (2026-02-11): Added persistent storage layer**
...
- **🔍 RESEARCH TOOLS (2026-02-11): Added web research capabilities**
...
- **📦 MODERN PACKAGING (2026-02-11): Updated project structure**
  - Added `pyproject.toml` for modern Python packaging
  - Added `requirements-dev.txt` for development dependencies
  - Added `quick_test_optimizer.py` for rapid testing
  - Reorganized documentation in `docs/` directory
- **🚀 CI/CD PIPELINE (2026-02-11): Added comprehensive quality gates**
...
- **Separation of concerns** (tokens ≠ costs ≠ logging ≠ orchestration ≠ UI ≠ database ≠ research)
