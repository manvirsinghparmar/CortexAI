...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. Work Package 2 adds billing accounts, provider snapshots, usage periods/counters/reservations, and webhook idempotency storage, but stored billing rows still do not authorize paid access or enforce allowances. Effective lifecycle policy and backend enforcement must remain server-side in later work packages, with verified Stripe webhooks ultimately authoritative for paid state.
...
- Subscription-plan or billing-class changes: `config/subscription_plans.yaml`, `server/billing/`, `config/model_registry.yaml`, `orchestrator/model_registry.py`, `/v1/models` schemas/routes, and matching backend/frontend contract tests.
