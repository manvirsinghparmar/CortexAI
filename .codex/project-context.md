...
- React Compare keeps every selected response visible in a responsive grid without horizontal response scrolling: three columns on wide desktop, two at tablet widths, and stacked cards on mobile. Cards use tall `640px–860px` desktop panels (`520px–720px` mobile), with fixed headers/footers and independently scrolling response bodies.
- Compare transcripts reserve bottom breathing room above the persistent composer so the input area does not compress the reading workspace. Ask turns remain content-sized.
...
- Mobile response cards collapse per-model latency, token, and cost metadata behind an icon-only chevron integrated beside the capability badge. Expansion reveals a compact muted strip within the header; desktop keeps metadata visible.
