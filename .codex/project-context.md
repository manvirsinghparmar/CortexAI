...
- Mobile response cards collapse completed per-model latency, token, and cost metadata behind an icon-only chevron integrated beside the capability badge. Expansion reveals a compact muted strip within the header; desktop keeps completed metadata visible. Pending and failed response cards keep their muted elapsed/status line visible on mobile and desktop. Response-card latency is displayed in seconds rounded to one decimal place while API DTOs remain `latency_ms`; unavailable token counts stay hidden instead of rendering zero-token placeholders.
