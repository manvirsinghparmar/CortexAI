...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. `server/billing/subscription_service.py` is the only effective-plan lifecycle authority and falls back conservatively to Free; `server/billing/entitlement_service.py` owns feature/model/file decisions and exact reservation quantities; `server/billing/metering_service.py` owns atomic reserve/settle/release/expiry transitions. `BILLING_ENABLED=false` is the safe default until verified Stripe webhook synchronization is deployed. Request routes are not yet wired to metering, and verified Stripe webhooks must ultimately be authoritative for paid state.
