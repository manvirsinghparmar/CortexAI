...
The current React client still uses multipart `POST /v1/files/upload-batch`.
That route and raw-byte `POST /v1/files/upload` remain compatibility contracts
while `ATTACHMENTS_LEGACY_PROXY_UPLOAD_ENABLED=true`. Both legacy routes keep
their current plan validation and SHA-256 same-user deduplication behavior.
