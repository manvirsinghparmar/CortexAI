...
- Ask and Compare resolve consumer-credit multipliers from each successful response's canonical requested model, while retaining provider-returned served and pricing identities for audit and provider-cost calculation. A versioned provider snapshot ID therefore cannot suppress sibling Compare ledger rows. If finalization still fails, the route immediately releases and unregisters the reservation so the heartbeat worker cannot keep credits indefinitely reserved.
