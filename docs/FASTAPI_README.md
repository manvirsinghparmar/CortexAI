...
- Frontend Compare selectors keep at least two active models and send only active selected models in compare requests. The offline fallback prefers `openai:gpt-5.6-luna` plus `claude:claude-sonnet-5`, and Add Model prefers `deepseek:deepseek-v4-flash`; normal authenticated startup replaces that fallback with selectable `/v1/models` rows. Remove controls appear with three active models and compact whichever two remain after any slot is removed.
- React manual Ask and Compare controls render through the same accessible provider-first model picker. It opens with provider logos and model counts, drills into only the selected provider's readable model labels, exact IDs, credit-use hints, locks, and active state, and provides Back to return to providers. The compact single-column flow and keyboard navigation are shared by desktop and mobile, while the viewport-positioned body portal prevents mobile Compare's horizontally scrollable model row from clipping it. Compare supplies duplicate-selection prevention and removal behavior; synchronized hidden native selects preserve existing Playwright selectors and `selectOption` flows.
