...
- Ask/Compare consumer-credit settlement uses the canonical requested model even when a provider reports a versioned served-model snapshot. Served/pricing identities remain available for response audit and provider-cost calculation, and one provider snapshot cannot prevent the other successful Compare targets from producing ledger rows. Any finalization failure releases and unregisters the reservation instead of leaving it heartbeat-active.
