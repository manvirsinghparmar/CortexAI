...
- Local Git gates: ensure [ci-commit-gate.md](/C:/Users/14169/PycharmProjects/PythonProject/OpenAIProject/.codex/ci-commit-gate.md)
  ...
  against the committed branch. Both hook stages isolate pytest from inaccessible
  user-wide Windows temp roots. Missing Docker defers only the API image build to
  GitHub Actions unless `CORTEX_CI_REQUIRE_DOCKER=1`. Do not bypass either hook for
  a normal handoff.
