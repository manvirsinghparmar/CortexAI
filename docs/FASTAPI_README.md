...
- Mobile response-card latency, token, and cost metadata starts collapsed behind an icon-only chevron beside the capability badge. Expanding it reveals a compact muted strip within the header; desktop metadata remains visible. The frontend displays latency in seconds rounded to two decimal places while API payloads remain `latency_ms`.
