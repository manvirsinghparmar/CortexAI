...
- Mobile response cards collapse per-model latency, token, and cost metadata behind an icon-only chevron integrated beside the capability badge. Expansion reveals a compact muted strip within the header; desktop keeps metadata visible. Response-card latency is displayed in seconds rounded to two decimal places while API DTOs remain `latency_ms`.
