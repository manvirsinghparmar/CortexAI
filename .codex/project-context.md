...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. `server/billing/subscription_service.py` is the only effective-plan lifecycle authority and falls back conservatively to Free; `server/billing/entitlement_service.py` owns feature/model/file decisions and exact reservation quantities; `server/billing/metering_service.py` owns atomic reserve/settle/release/expiry transitions; `server/billing/enforcement_service.py` composes those services for DB-mode Ask/Compare. Ask/Compare sync and stream routes reserve before provider calls, settle only actual successful model classes and performed research, release failures, and partially settle Compare. Smart routing filters by plan billing class without changing its independent tier. `BILLING_ENABLED=false` still resolves every account to Free, and verified Stripe webhooks must ultimately be authoritative for paid state. Optimize/file/export enforcement remains later work.
