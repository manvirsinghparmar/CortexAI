...
- Consumer plans are server-owned in `config/subscription_plans.yaml` and must validate at API startup. Model billing classes (`standard`, `advanced`, `ultra`) are distinct from smart-routing tiers (`T0`-`T3`); missing legacy classifications fall back conservatively to `advanced`, while unknown values fail registry loading. `server/billing/subscription_service.py` is the only effective-plan lifecycle authority and falls back conservatively to Free; `server/billing/entitlement_service.py` owns feature/model/file decisions and exact reservation quantities; `server/billing/metering_service.py` owns atomic reserve/settle/release/expiry transitions; `server/billing/enforcement_service.py` composes those services for DB-mode Ask/Compare/Optimize/upload. Ask/Compare sync and stream routes reserve before provider calls, settle only actual successful model classes, performed research, and one attachment-analysis turn per billable attachment-backed action, release failures, and partially settle Compare. The explicit `/v1/optimize` UI path settles one started optimizer invocation and is not recounted by the later Ask/Compare submission. Upload reserves the observed body size before storage, settles successful `uploaded_bytes`, and releases failures. Smart routing filters by plan billing class without changing its independent tier. `BILLING_ENABLED=false` still resolves every account to Free, verified Stripe webhooks must ultimately be authoritative for paid state, and usage-export enforcement remains later work.
- Stripe hosted billing sessions keep credentials, paid-plan Price IDs, redirects, provider payloads, and response validation in `server/billing/stripe_gateway.py`. `server/billing/session_service.py` separates provider calls from short DB transactions, atomically claims Customer identity, and sends provider-live accounts to Portal instead of duplicate Checkout. Strict request schemas reject client Price/amount/currency/Customer/redirect fields. `BILLING_ENABLED=false` makes hosted-session routes return `billing_not_configured`; hosted-session creation never grants paid access, and verified Stripe webhooks remain authoritative.
...
- Effective subscription endpoint: `server/routes/entitlements.py`; lifecycle/decision services: `server/billing/`
