...
- Desktop sidebar history uses compact two-line rows: one ellipsized title line and one metadata line containing only mode and date. Keep model names, turn counts, and token counts hidden, preserve the full title in the row tooltip, and leave the separate mobile History layout unchanged. The desktop sidebar has an icon-only top-right collapse control that reduces it to a narrow action rail; Ask, Compare, Usage, and Models live in the desktop rail; the bottom-left session tile is status-only for active sessions and must not trigger logout; the mobile bottom navigation remains Ask/Compare/History only while Usage and Models are reached from the account menu.
