...
def test_registry_exposes_explicit_billing_classes_separate_from_routing_tiers():
    ...
    ultra = registry.find_model("claude", "claude-opus-4-6")
    ...
    assert ultra.billing_class is ModelBillingClass.ULTRA


def test_registry_missing_billing_class_uses_conservative_fallback(tmp_path, caplog):
    ...
    registry = ModelRegistry.from_yaml(str(path))
    candidate = registry.find_model("openai", model["name"])

    assert candidate is not None
    assert candidate.billing_class is ModelBillingClass.ADVANCED
    assert "conservative advanced fallback" in caplog.text
