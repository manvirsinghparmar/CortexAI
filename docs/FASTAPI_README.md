...
- Mobile completed response-card duration, token, and cost metadata starts collapsed behind an icon-only chevron beside the capability badge. Expanding it reveals a compact muted strip within the header; desktop completed metadata remains visible. Loading and failed cards keep a muted elapsed/status line visible on mobile and desktop. The frontend displays the same UI-observed elapsed duration in seconds when live timestamps are available, falls back to API `latency_ms` for restored rows, and keeps unavailable token counts hidden instead of rendering zero-token placeholders.
...
- With the frontend Improve toggle enabled, the user-message slot first shows a stable, reduced-motion-aware refinement state, then the returned `optimized_prompt` fades into place; when `was_optimized=false` or refinement is unavailable, the original prompt is shown with a short soft-success reassurance note before that prompt is sent to Ask or Compare.
