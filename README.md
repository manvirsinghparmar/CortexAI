...
- Mobile response cards keep latency, token, and cost metadata collapsed by default behind an icon-only chevron integrated beside the capability badge. Tapping it expands a compact muted strip within the header; desktop keeps response metadata visible. Response-card latency displays in seconds rounded to two decimal places.
...
- Frontend Compare response cards use side-by-side model columns with visible per-card latency in seconds and token metadata on desktop; mobile exposes the same metrics through the compact header disclosure and phone-sized Compare switches between model responses with tabs.
