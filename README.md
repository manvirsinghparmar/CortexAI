...
- On desktop, each React Compare response column has its own vertical response scrollbar while the comparison canvas handles horizontal overflow. Mobile replaces stacked or horizontally scrolling response cards with a segmented model switcher and shows one response at a time in natural page flow.
...
- Mobile response cards hide latency, token, and cost metadata by default and expose it through a per-card `Details` control; desktop keeps response metadata visible.
