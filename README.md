...
- React manual Ask and Compare model controls share one accessible provider-first picker. It opens on provider names, logos, and model counts, then shows only the chosen provider's readable model-family labels, exact API IDs, credit-use hints, plan locks, and active state; Back returns to the provider list. The same compact drill-down is used on desktop and mobile, with arrow-key navigation and a viewport-contained body portal. Compare adds duplicate-option disabling and removal rules through the shared component API. Hidden native selects remain synchronized for browser automation compatibility.
