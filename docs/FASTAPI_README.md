...
- React Compare keeps every selected response visible in a responsive grid without horizontal response scrolling: three columns on wide desktop, two at tablet widths, and stacked cards on mobile. Cards use tall `640px–860px` desktop panels (`520px–720px` on mobile) with independent response-body scrolling.
- Model headers and action footers remain fixed inside each Compare card while only the answer body scrolls. The transcript reserves bottom breathing room above the persistent composer so the input area does not compress the reading workspace.
...
- Mobile response-card latency, token, and cost metadata starts collapsed behind an icon-only chevron beside the capability badge. Expanding it reveals a compact muted strip within the header; desktop metadata remains visible.
