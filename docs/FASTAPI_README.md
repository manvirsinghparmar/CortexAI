...
- Mobile completed response-card latency, token, and cost metadata starts collapsed behind an icon-only chevron beside the capability badge. Expanding it reveals a compact muted strip within the header; desktop completed metadata remains visible. Loading and failed cards keep a muted elapsed/status line visible on mobile and desktop. The frontend displays completed latency in seconds rounded to one decimal place while API payloads remain `latency_ms`, and unavailable token counts stay hidden instead of rendering zero-token placeholders.
