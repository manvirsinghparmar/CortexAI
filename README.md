...
- React Compare keeps every selected response visible in a responsive grid without a horizontal response rail: three columns on wide desktop, two columns at tablet widths, and stacked cards on mobile. Each card uses a tall `640px–860px` desktop reading panel (`520px–720px` on mobile) with an independently scrolling response body.
- Compare card headers and action footers remain fixed inside each panel while only the answer body scrolls. The transcript retains bottom breathing room above the persistent composer so the input area does not compress the reading workspace.
...
- Mobile response cards keep latency, token, and cost metadata collapsed by default behind an icon-only chevron integrated beside the capability badge. Tapping it expands a compact muted strip within the header; desktop keeps response metadata visible.
...
- Frontend Compare response cards use side-by-side model columns with visible per-card latency and token metadata on desktop; mobile exposes the same metrics through the compact header disclosure.
