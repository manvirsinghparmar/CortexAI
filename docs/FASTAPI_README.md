...
`GET /v1/credits/transactions?limit=100&offset=0` returns the authenticated account's newest immutable reconciliation rows. Model and research items include the operation, provider/model where applicable, input/output tokens and credits, fixed credits, total credits, provider cost, estimated-usage flag, pricing version, metadata, and timestamp.
